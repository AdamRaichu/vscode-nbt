...
export function getMcmetaRoot(ctx: vscode.ExtensionContext) {
  return vscode.Uri.joinPath(ctx.globalStorageUri, "cache", "mcmeta");
}
...
export async function getAssets(dataVersion: number | undefined, logger: Logger, ctx: vscode.ExtensionContext) {
  const downloader = new Downloader(getMcmetaRoot(ctx), logger);
      ...
      cache: cacheOptions(async (data) => {
        ...
        const filterEntries = (type: string) => {
          ...
          return Object.fromEntries(
            entries.flatMap<[string, unknown]>(({ path, data }) => {
